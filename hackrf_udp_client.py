...
class HackRFClient:
    ...
    def start_stream(self, hackrf_args):
        ...
        try:
            ...
            if response_data.get('status') == 'stream_started':
                ...
                print(f"✗ Stream start failed: {response_data.get('error', 'Unknown error')}")
                return False
        ...
        try:
            ...
            if stats['clients']:
                ...
                for client in stats['clients']:
                    ...
                    print(f"    - {client['address']} (connected {duration_mins:.1f}m ago)")
    ...
    def listen_to_stream(self, duration=None):
        ...
        print(f"Listening to stream{'...' if duration is None else f' for {duration} seconds...'}")
        ...
        if self.connected:
            try:
                    ...
                    try:
                        ...
                        if response == b"DISCONNECTED":
                            ...
                            if decoded.startswith('{') or 'DISCONNECT' in decoded:
                                ...
                                if attempts == 1:
                                    print("⏳ Waiting for disconnect confirmation (still receiving data)...")
                                continue
                ...
                if not disconnect_confirmed and attempts >= max_attempts:
                    print("⚠️ Disconnect response unclear, but proceeding with disconnection")
...
Examples:
  ...
  python3 hackrf_udp_client.py --host localhost --interactive
    ...
    parser.add_argument('--start-stream', action='store_true', help='Start a stream with remaining args')
    parser.add_argument('--listen', type=int, metavar='SECONDS', help='Listen to stream for N seconds')
            ...
            while True:
                try:
                    ...
                    if cmd == 'quit' or cmd == 'exit':
                        ...
                        args_input = input("HackRF args (e.g., -f 88:108 -g 20 -l 16): ").strip()
                        if args_input:
                        ...
                        duration_input = input("Duration in seconds (or press Enter for indefinite): ").strip()
